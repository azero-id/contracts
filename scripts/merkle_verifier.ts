...
const abi = JSON.parse(readFileSync('./target/ink/merkle_verifier/metadata.json', 'utf-8'))
 ...
 */
const instantiate = async (tree: MerkleTree): Promise<{ contractAddress: string }> => {
  ...
  let contractAddress
  await code.tx
    .new({ gasLimit: 1000n * 1000000n, storageDepositLimit: null }, rootUint8Array)
  ...
  if (result.isOk && !!output) {
    ...
    const arraysAreEqual = JSON.stringify(valueU8a) === JSON.stringify(rootUint8Array)
    if (!arraysAreEqual)
      console.error('On-chain Uint8Array does not equal MerkleTree root:', rootUint8Array, valueU8a)
    else console.log('Successfully stored & queries MerkleTree root from contract.')
  } else {
