...
fn get_storage_struct(items: &Vec<Item>) -> ItemStruct {
...
pub fn azns_contract(args: TokenStream, input: TokenStream) -> TokenStream {
    let args = parse_macro_input!(args as AznsArgs);
    let mut module = parse_macro_input!(input as ItemMod);

    let mut module_items = get_module_items(&mut module);
    let storage_name = get_storage_struct(&module_items).ident;

    if let Some(data) = args.ownable2step {
        let (err_name, err_val) = data;
        // verify admin, pending_admin fields ??
        support_ownable2step(&mut module_items, &storage_name, &err_name, &err_val);
    }
    if args.upgradable {
        support_upgrade(&mut module_items, &storage_name);
    }
